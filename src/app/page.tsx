    ...
    <AppShell>
      ...
      <div id="agents" className="mb-12">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-white text-xl font-semibold">Agents</h2>
              ...
              <div key={agent.id} className="bg-white/[0.06] backdrop-blur-xl rounded-2xl p-6 border border-white/[0.1] hover:border-emerald-500/20 transition-all group">
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {model && (
                      <p className="text-white/25 text-xs mt-1 font-mono">{formatModel(model)}</p>
                    )}
                  </div>
                </div>
            ...
            <div className="bg-white/[0.06] backdrop-blur-xl rounded-2xl border border-white/[0.1] overflow-hidden">
              <div className="divide-y divide-white/[0.06]">
                {tasks.slice(0, 12).map(task => {
                  ...
                  return (
                    <div key={task.id} className="p-4 hover:bg-white/[0.03] transition-colors">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <p className={`text-sm font-medium mb-1 ${task.status === 'done' ? 'text-white/30 line-through' : 'text-white/90'}`}>
                            {task.task}
                          </p>
                          {task.notes && (
                            <p className="text-white/30 text-xs mb-2">{task.notes}</p>
                          )}
              ...
              <div className="divide-y divide-white/[0.06]">
                {blockers.map(b => (
                  <div key={b.id} className="p-4">
                    <p className="text-white/80 text-sm leading-relaxed mb-2">{b.description}</p>
